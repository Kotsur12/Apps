...
class MyPDO
{
    ...
    public function insert($name, $isShow,$imageLink)
    {
        ...
        $sql = 'INSERT INTO `category` (`name`, `priority`, `is_show`, `image`) VALUES (?, ?, ?, ?)';
        $stmt = $this->_pdo->prepare($sql);
        $stmt->execute([$name, $priority, $isShow, $imageLink]);
    }
...
if ((isset($_REQUEST['action'])) && isset($_REQUEST['cat_name']) && isset($_REQUEST['cat_isShow']) && isset($_REQUEST['cat_imageLink'])) {
    switch ($_REQUEST['action']) {
        case "add":
...
<head>
    ...
    <script type="text/javascript">
        function onUpButtonClick(id) {
            location.href = location.origin + "/SportBettings/webinterface.php" + "?action=up&cat_id=" + id;
        }

        function onDownButtonClick(id) {
            location.href = location.origin + "/SportBettings/webinterface.php" + "?action=down&cat_id=" + id;
        }

        function onAddButtonClick(name, isShow, imageLink) {
            location.href = location.origin + "/SportBettings/webinterface.php" + "?action=add&cat_name=" + name + "&cat_isShow=" + isShow + "&cat_imageLink=" + imageLink;
        }

        function onDeleteButtonClick(id) {
            location.href = location.origin + "/SportBettings/webinterface.php" + "?action=delete&cat_id=" + id;
        }

        function onAddEventClick(id) {
            location.href = location.origin + "/SportBettings" + "/events.php?cat_id=" + id;
        }

        function onShowClick(id) {
            location.href = location.origin + "/SportBettings/webinterface.php" + "?action=show&cat_id=" + id;
        }
...
<form action='' method='GET' align="center">
    ...
    <input id="imageLink" type='text' placeholder="Image link" name="imageLink"/>
    <div onclick="onAddButtonClick(document.getElementById('name').value, document.getElementById('isShow').value,document.getElementById('imageLink').value)"
         class="buttonAdd">
        Add Sport
    </div>
</form>
